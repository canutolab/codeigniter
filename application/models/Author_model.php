<?php 
	...
	*/
	class Author_model extends CI_Model
	{
		...
		public function getAuthorByContry(int $country){
			$this->db->select("CONCAT(a.ultimo_nome, ', ', a.primeiro_nome) AS autor");
			$this->db->from("biblioteca.autores a");
		...
		public function getAll(){
			$this->db->select("id, CONCAT(a.ultimo_nome, ', ', a.primeiro_nome) AS autor");
			$this->db->from("biblioteca.autores a");
			// $this->db->where("a.nacionalidades_id",$country);
			// $this->db->like("a.primeiro_nome", "o", "both");//tem o no primeiro nome e coisas a volta desse o
			// $this->db->not_like("a.primeiro_nome", "x", "both");//nao tem x no nome
		...
		public function getListValidId(){
			$this->db->select('id');
			$query = $this->db->get('autores')->result_array();
			var_dump($query);
			//$list =" ". implode(",", $query);
			// $list;
		}
